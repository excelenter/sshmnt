...
# sshmnt.sh
#
# Script that automatically mounts ssh directories into $SSHFSROOT 
# directory.
#
...
if [ ! $1 ] 
then
	echo "${info}Usage: ${txtbld}$(basename $0) ${txtrst}${txtemph}[user@server.com]${txtrst}"
	exit
fi
...
if sshfs "$1": "$SSHFSROOT$1" 2> /dev/null
then
